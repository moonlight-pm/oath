...
QEMU never packs Steam / Xwayland / mesa / gamescope.
System D-Bus + BlueZ are `pkg:bluez` (menubar Bluetooth chip; not
session MPRIS). T36 remaining kit apps closed (same `pkg:sola` blob; no new `svc`;
...
(not a kind): `https://store.oath.wicket.cloud` **200** (Wicket
`extras/oath-store`, `wl-c3222462d626`). Serving `/oath/store` from an
Oath guest, signatures, and discovery are deferred. Content-hash
identity is T32 (in). QEMU is parked.
