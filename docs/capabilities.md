...
| ssh | Home SSH via catalog keys | partial | [freeze](specs/2026-09-02-seat-home.md) | QEMU probe login/deny/sudo as `home`; **canto `ssh` / `scp` / `sftp` as `home@canto` (uid 1)** | dropbear `-w`; uid 1; sudo ALL no password; groups `root`+`home`; interactive PTY (`ptmxmode=0666`); no baked private key; `pkg:dropbear` includes `/bin/scp` + `/bin/sftp-server` (canto live 2026-09-03; next `cargo make build` for QEMU). Editor is busybox `/bin/vi` | [ssh](manual/ssh.md) |
