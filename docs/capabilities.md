...
| sola | Sola session on Oath | partial | [freeze](specs/2026-09-02-seat-home.md) | QEMU session probe (stack as `home` uid 1); canto T31 SSH + River/Sola as `home` (uid 1, amdgpu DP-10, gles2/radeonsi) | T31 QEMU + canto metal: graphical stack as `home`; kit ELFs in `pkg:sola` packed; virtio-only SW cursor + pixman + `LIBGL_ALWAYS_SOFTWARE`; metal: gles2/radeonsi; no git/grok/rustc on the guest; wrapper/mail/other kit apps out; no dbus-daemon | [services](manual/services.md) |
