...
| boot | Kernel + init + QEMU | partial | [plan](plans/2026-08-27-qemu-skeleton-plan.md) | `cargo make run` / `probe`; canto EFI splash | borrowed kernel; metal BOOTX64 is `oath-efi` (native GOP, white mark, LoadImage kernel+initrd). Linux does not paint fb (amdgpu mmap wedged Pitcairn). Gaps: Apple firmware still first; GOP may blank when amdgpu binds; QEMU appliance still `-kernel` (no EFI splash unless OVMF install) | [qemu](manual/qemu.md) |
