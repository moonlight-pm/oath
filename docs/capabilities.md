...
| pkg | Package objects | partial | [freeze](specs/2026-09-02-pkg-grok.md) | QEMU probe + fetchme; canto `pkg:grok` | T20 identity: `pkg.url`, peer Oath host as origin; **T30** `pkg:grok` packed (catalog owns ELF; Grok does not self-update); no guest store export, hash, deps, versions, or url-refetch; `pkg:git` not packed | [packages](manual/packages.md) |
...
| sola | Sola session on Oath | partial | [freeze](specs/2026-09-02-seat-home.md) | QEMU session probe (stack as `home` uid 1); canto T31 SSH + River/Sola as `home` (uid 1, amdgpu DP-10, gles2/radeonsi); oath-sola `4a210e32` packed live | T31 QEMU + canto metal: graphical stack as `home`; kit ELFs in `pkg:sola` packed (Sola master: Super+Tab counts, notify pile, spectrum, rounded floats, browser omnibox/devtools); virtio-only SW cursor + pixman + `LIBGL_ALWAYS_SOFTWARE`; metal: gles2/radeonsi; `pkg:grok` packed (git/rustc still host-side); wrapper/mail/spotify/other kit apps out; no dbus-daemon | [services](manual/services.md) |
