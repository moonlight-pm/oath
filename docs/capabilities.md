...
| ssh | Home SSH via catalog keys | partial | [freeze](specs/2026-09-02-seat-home.md) | QEMU probe login/deny/sudo/undo/reboot as `home`; **canto `ssh home@canto`** | dropbear `-w`; `home` only; sudo ALL no password; no baked private key; canto serial svc off (no UART) | [ssh](manual/ssh.md) |
