...
| catalog | Live INDEX + schemas | partial | [freeze](specs/2026-08-27-catalog-and-oath-surface.md) | QEMU serial | INDEX generated; not e2e-proofed after reboot | [qemu](manual/qemu.md) limited |
| objects | Typed object store | partial | freeze | QEMU | host/svc/snap only | no |
| oath-cli | `oath` verbs | partial | freeze | QEMU | MCP later; `--from-json` not in INDEX | limited |
| snap | generations; apply / undo | partial | freeze | live undo of hostname | reboot-generation e2e; nested subvol layout still crude | no |
| svc | Own supervisor | partial | freeze | `svc:serial` | notify socket barely used; no wants-order | no |
| boot | Kernel + init + QEMU | partial | [plan](plans/2026-08-27-qemu-skeleton-plan.md) | `image/run.sh` | borrowed kernel+modules; initramfs module load; sudo to pack disk | [qemu](manual/qemu.md) |
