...
| svc | Own supervisor | partial | freeze | QEMU: hold+sshd+seatd+river+sola; canto: sshd (serial/river/Sola off) | no Requires-style hard deps; canto `svc:serial` off (no UART); canto graphical svcs off | [services](manual/services.md) |
...
| sola | Sola session on Oath | partial | [freeze](specs/2026-09-02-seat-home.md) | QEMU session probe (stack as `home` uid 1); canto T31 SSH as `home`, **graphical svcs off** | T31 QEMU: graphical stack as `home`; canto metal: river/Sola/seatd disabled (crash-loop as `home` on amdgpu — not re-enabled yet); kit ELFs in `pkg:sola` packed; virtio-only SW cursor + `LIBGL_ALWAYS_SOFTWARE`; no git/grok/rustc on the guest; no radeonsi; wrapper/mail/other kit apps out; no dbus-daemon | [services](manual/services.md) |
