...
| backup | Off-box generation (NFS) | partial | [freeze](specs/2026-09-03-backup-nfs.md) | canto gen 16 → nas `10.0.0.12:/mnt/alpha/backup/canto` (`canto.send` + sidecar; checksum match) | T33: one overwrite send; helper + `svc:backup` seeded (enabled=false, `restart=never`); NFS client packed for next image. Gaps: canto this boot insmod’d nfs by hand (ESP initrd old); QEMU next `cargo make build`; no restore; no hourly/retention | [services](manual/services.md) |
