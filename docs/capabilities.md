...
| boot | Kernel + init + QEMU | partial | [plan](plans/2026-08-27-qemu-skeleton-plan.md) | `cargo make run` / `probe`; canto EFI splash | borrowed kernel; metal BOOTX64 is `oath-efi` (native GOP, white mark, LoadImage kernel+initrd). amdgpu loads just before River; `drm_kms_helper.fbdev_emulation=0` so no text console. Gaps: Apple firmware still first; a modeset flash as amdgpu binds is possible; QEMU appliance still `-kernel` | [qemu](manual/qemu.md) |
...
| sola | Sola session on Oath | partial | [freeze](specs/2026-09-01-sola-browser.md) | QEMU session probe; canto HDMI (amdgpu Pitcairn, pixman, 1920×1080@75) + live `sola-browser` | session stack packed; `sola-terminal` + `sola-browser` (CEF) in `pkg:sola`; kit seed fonts (SF Pro Text + Iosevka Term Slab; Inter / JetBrains Mono fallbacks); `run-compositor` picks the connected DRM card; sola-river picks the mode matching physical aspect (DualUp 2560×2880@30 vs 4K@60); software GL; no amdgpu accel; wrapper/mail/other kit apps out; no dbus-daemon; QEMU image needs rebuild for browser/fonts | [services](manual/services.md) |
