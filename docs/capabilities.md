...
| sola | Sola session on Oath | partial | [freeze](specs/2026-09-02-seat-home.md) | QEMU session probe (stack as `home` uid 1); canto T31 SSH + River/Sola as `home` (uid 1, amdgpu DP-10) | T31 QEMU + canto metal: graphical stack as `home`; kit ELFs in `pkg:sola` packed; virtio-only SW cursor + `LIBGL_ALWAYS_SOFTWARE`; no git/grok/rustc on the guest; no radeonsi (Mesa dri2 warn, shell still paints); wrapper/mail/other kit apps out; no dbus-daemon | [services](manual/services.md) |
