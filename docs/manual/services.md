...
| `svc:sola-kvm` | `/bin/sola-kvm listen` | enabled, `restart=always` | Linux KVM **client** as `home`. UDP 4242 injects via River virtual pointer/keyboard. CLIP1 TCP on the same port syncs text and `image/png` on Enter/Leave. Wants `svc:river`. |
