...
| `pkg:xwayland` | `present: true` | yes — Debian Xwayland 24.1.13; `/bin/Xwayland`. Rootful nest for Steam (`DISPLAY=:2`, `-geometry`, `-decorate`, `-glamor es`). Needs `xkbcomp` in the pack and `/usr/share/X11/xkb` → river’s xkeyboard-config. gamescope also looks this up on PATH. |
| `pkg:gamescope` | `present: true` | yes — windowed nest compositor; `/bin/gamescope`. Arcade Play uses `--backend wayland`. Wrapper sets `VK_LAYER_OATH_gamescope_pool` (pads Ubuntu 3.16's short combined-image-sampler pool so RADV SI `vkAllocateDescriptorSets` succeeds) and host nodes `/usr/bin/Xwayland` + `/usr/share/gamescope`. Canto: RADV PITCAIRN is selected, nested Xwayland starts, then River kills the client (`xdg_surface.set_window_geometry(0,0,0,0)` — libdecor fallback, no cairo plugin). Steam currently uses rootful Xwayland instead. |
| `pkg:mesa` | `present: true` | yes — 64-bit OpenGL/GLX + Vulkan WSI (Debian mesa 26.1.6 RADV, glvnd, Khronos loader 1.4.357). `/bin/vulkaninfo`. ICD `share/vulkan/icd.d/radeon_icd.json` (64-bit) and `radeon_icd32.json` (32-bit Steam). gamescope/vulkaninfo set `VK_ICD_FILENAMES` to the 64-bit json. LLVM 64-bit in `pkg:river`; 32-bit LLVM 21 in `pkg:mesa/lib32`. |
| `pkg:steam` | `present: true` | yes — Valve steam-launcher + ubuntu12_32 bootstrap + 32-bit loader in `lib32` + 64-bit steamrt3 SONAMEs in `lib64`. `/bin/steam` (wrapper). User state is `~/.steam` and `~/.local/share/Steam`. The wrapper creates host nodes Steam’s scripts assume: `/usr/bin/env` + `/usr/bin/bash`, `/lib64/ld-linux-x86-64.so.2`, `/lib/ld-linux.so.2` + `/lib/i386-linux-gnu`, `/etc/ssl/certs/ca-certificates.crt`, `/bin/ldd`. Busybox `xz`/`tar` lack `--robot` / `--blocking-factor`; shims live in `pkg:steam/libexec` and are prepended to PATH. 32-bit `libGL` (and gtk/pulse) come from the user’s steamrt3c i386 tree, SONAME-linked beside `steamui.so` — putting 64-bit `pkg:sola` `libGL` on `LD_LIBRARY_PATH` is `wrong ELF class: ELFCLASS64`. Direct `steam` nests on rootful `pkg:xwayland` (`DISPLAY=:2`). steamwebhelper does **not** use pressure-vessel (`CLONE_NEWUSER` is EPERM after PID 1 chroot); `/bin/steam` sets `STEAM_RUNTIME_STEAMRT` to `pkg:steam/libexec/pv-host` and 64-bit GLX from `pkg:mesa`. Arcade Play still wants gamescope. |
