...
| `pkg:sola` | `present: true` | yes — session stack + `sola-terminal`; `/bin/sola-bus` and siblings including `sola-session`, `sola-terminal`, and `tmux` (C.UTF-8 locale-archive + JetBrains Mono in the store tree) |
