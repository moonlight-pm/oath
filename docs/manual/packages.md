...
| `pkg:gamescope` | `present: true` | yes — windowed nest compositor; `/bin/gamescope` (`--help` works). Arcade Play uses `--backend wayland`. Canto: RADV PITCAIRN is selected, then `vkAllocateDescriptorSets` fails (SI); Steam currently uses rootful Xwayland instead. |
