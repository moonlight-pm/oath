...
| `pkg:mesa` | `present: true` | yes — 64-bit OpenGL/GLX (Debian mesa 26.1.6 + glvnd). No `/bin` ELF. `LIBGL_DRIVERS_PATH` is `pkg:mesa/lib/dri` (libdril → radeonsi). LLVM stays in `pkg:river`. steamwebhelper sets `__GLX_VENDOR_LIBRARY_NAME=mesa`. |
