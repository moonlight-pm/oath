...
product tree (`oath build`). It does not link `/bin`. Canonical
example: `apps/hello.plan` (compile `pkg:hello-src` with declared
`pkg:cc` + `pkg:busybox`).
...
| `pkg:sola` | `present: true` | yes — session stack + `sola-terminal` + `sola-browser` + `sola-workspaces` + `sola-kvm` + **`sola-paint`** / **`sola-preview`**; `/bin/sola-bus` and siblings including `sola-session`, `sola-terminal`, `sola-browser`, `sola-workspaces`, `solactl`, `sola-kvm`, `tmux`, `/bin/xdg-open` and `/bin/open` (`solactl open` → sola-browser or sola-paint for images; `x-www-browser` is the same shim; not xdg-utils). C.UTF-8 locale-archive; CEF under `cef/`; SF Pro Text + Iosevka Term Slab, with Inter / JetBrains Mono fallbacks. **`sola-arcade`** stays in this blob; `/bin/sola-arcade` is linked only when the connected GPU has Vulkan WSI DRM modifiers (not on canto Pitcairn). **T40** `/bin/sola-oath` is this blob (catalog client; canto live-copied 2026-09-13; launcher builtin waits on packed sola-shell). Remaining T36 names (`sola-settings`, mail, spotify, wrapper, …) still not in `/bin` on canto. |
