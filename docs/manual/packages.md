...
| `pkg:steam` | `present: true` | yes — Valve steam-launcher + ubuntu12_32 bootstrap + 32-bit loader in `lib32`. `/bin/steam` (wrapper). User state is `~/.steam` and `~/.local/share/Steam`. The wrapper creates host nodes Steam’s scripts assume: `/usr/bin/env` + `/usr/bin/bash` (steam.sh shebang), `/lib64/ld-linux-x86-64.so.2` (64-bit srt-logger interp), `/lib/ld-linux.so.2` + `/lib/i386-linux-gnu` (32-bit client), `/etc/ssl/certs/ca-certificates.crt`, and `/bin/ldd`. Missing `/usr/bin/env` or `/lib64` is the kernel `cannot execute: required file not found` on `steam`. |
