...
| `pkg:sola` | `present: true` | yes — session stack + `sola-terminal` + `sola-browser`; `/bin/sola-bus` and siblings including `sola-session`, `sola-terminal`, `sola-browser`, and `tmux` (C.UTF-8 locale-archive; CEF under `cef/`; SF Pro Text + Iosevka Term Slab, with Inter / JetBrains Mono fallbacks) |
