...
| `pkg:gamescope` | `present: true` | yes — windowed nest compositor; `/bin/gamescope` (`--help` works). Arcade Play uses `--backend wayland`. Canto: RADV PITCAIRN is selected, then Vulkan WSI still fails; Steam currently uses rootful Xwayland instead. |
