...
1. **T31 on canto: `ssh home@canto`.** uid 1, `HOME=/home`, sudo ALL
   ...
   **gamescope** (`--backend wayland --steam`, no `-b`, no
   ...
   `image/oath-cc-link.sh` + `zig-gnu-cc.sh`. Next: --steam restored (no-overlay strobed, xdg never configured). One-surface via unmap extras, not dropping --steam. Arcade
