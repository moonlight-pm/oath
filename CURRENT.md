...
1. **T31 on canto: `ssh home@canto`.** uid 1, `HOME=/home`, sudo ALL
   ...
   **`pkg:thoxa`** is the `home` login shell (`/bin/thoxa`; `/etc/shells`
   ...
   loads with 32-bit GL from steamrt3c). Direct `steam` nests in
   rootful `pkg:xwayland` (`DISPLAY=:2`; xkbcomp + `/usr/share/X11/xkb`).
   ...
   1.4.357, `/bin/vulkaninfo`). `gamescope --backend wayland` selects
   `AMD Radeon Graphics (RADV PITCAIRN)` and allocates descriptors
   (`VK_LAYER_OATH_gamescope_pool` pads Ubuntu 3.16's short YCbCr
   ...
   **Steam this boot:** `/bin/steam` stays up. Rootful Xwayland `:2`,
   ...
   popup (`Sign in to Steam`) is on `:2`. steamui identifies the
   ...
   dual ICD jsons; `hosts: files dns` + `steamloopback.host`.
   Still logs missing `VK_KHR_surface` / `VK_KHR_xlib_surface`.
   ...
   **T36** other kit names still out (`alsa.pc`). Zig `cc` host link is
   `image/oath-cc-link.sh` + `zig-gnu-cc.sh`. Next: 32-bit Vulkan WSI
   (`VK_KHR_surface`), Arcade Play smoke, or a `.pc` for alsa.
