...
1. **T31 on canto: `ssh home@canto`.** uid 1, `HOME=/home`, sudo ALL
   ...
   **`pkg:thoxa`** is the `home` login shell (`/bin/thoxa`; `/etc/shells`
   ...
   loads with 32-bit GL from steamrt3c). Direct `steam` then exits
   `Unable to open X11 display` (host River has no XWayland). **T36**
   other kit names still out (`alsa.pc`). Zig `cc` host link is
   `image/oath-cc-link.sh` + `zig-gnu-cc.sh`. Next: Arcade Play via
   gamescope (needs gamescope NEEDED vs packed glibc), or a `.pc`
   for alsa / remaining kit ELFs. `CLONE_NEWUSER` is EPERM (srt-bwrap
   stubbed).
