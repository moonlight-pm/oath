...
1. **T31 on canto: `ssh home@canto`.** uid 1, `HOME=/home`, sudo ALL
   ...
   **gamescope** (`--backend wayland --steam`, no `-b`, no
   ...
   `image/oath-cc-link.sh` + `zig-gnu-cc.sh`. Next: nest Fit poke (GAMESCOPE_XWAYLAND_MODE_CONTROL + FORCE_WINDOWS_FULLSCREEN property, sola-arcade path) live at 1920×1080. Then RADV export-only linear. Arcade
