...
1. **T31 on canto: `ssh home@canto`.** uid 1, `HOME=/home`, sudo ALL
   ...
   **`pkg:thoxa`** is the `home` login shell (`/bin/thoxa`; `/etc/shells`
   ...
   `gamescope --help` works; Wayland nest still dies on RADV WSI.
   **T36** other kit names still out (`alsa.pc`). Zig `cc` host link is
   `image/oath-cc-link.sh` + `zig-gnu-cc.sh`. Next: Steam CEF/webhelper
   64-bit GL, or gamescope Vulkan WSI, or a `.pc` for alsa. `CLONE_NEWUSER`
   is EPERM (srt-bwrap stubbed).
