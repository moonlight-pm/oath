...
1. **T31 on canto: `ssh home@canto`.** uid 1, `HOME=/home`, sudo ALL
   ...
   **`pkg:thoxa`** is the `home` login shell (`/bin/thoxa`; `/etc/shells`
   ...
   `THOXA_ROOT`). `ssh home@canto echo hi` is `thoxa -c`. sola-terminal
   and workspaces tmux inherit `$SHELL` (pkg:sola wrappers default
   `/bin/thoxa`, not ash; wrappers patched this boot). Root/serial
