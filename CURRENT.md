...
1. **T31 on canto: `ssh home@canto`.** uid 1, `HOME=/home`, sudo ALL
   ...
   **`pkg:thoxa`** is the `home` login shell (`/bin/thoxa`; `/etc/shells`
   ...
   pool). Nested Xwayland starts; River then kills the client
   (`xdg_surface.set_window_geometry(0,0,0,0)` — libdecor fallback,
   no cairo plugin). Direct `steam` still uses rootful Xwayland.
   **T36** other kit names still out (`alsa.pc`). Zig `cc` host link is
   `image/oath-cc-link.sh` + `zig-gnu-cc.sh`. Next: libdecor-cairo so
   the nest window is non-zero, or a `.pc` for alsa.
