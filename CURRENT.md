...
1. **T31 on canto: `ssh home@canto`.** uid 1, `HOME=/home`, sudo ALL
   ...
   **gamescope** (`--backend wayland --steam`, no `-b`, no
   ...
   `image/oath-cc-link.sh` + `zig-gnu-cc.sh`. Next: nest fullscreen paints; DPM forced high (was 300/150). Confirm fps/flip. Layer no longer blit-waits GetMemoryFdKHR (relaunch). Arcade
